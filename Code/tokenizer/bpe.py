...
class ASCIIBPETokenizer:
    ...
    def merge(self, token_ids: list[int]) -> list[int]:
        ...
        """
        # Step 1: Compute bigram statistics
        # bigram_counts = compute_bigram_statistics(token_ids)

        # if not bigram_counts:
        #     return token_ids  # No bigrams to merge
        
        # # Step 2: Find the most frequent bigram (lexicographically smaller in case of tie)
        # most_frequent_bigram = min(bigram_counts.items(), key=lambda x: (-x[1], x[0]))[0]

        # # Step 3: Create new token for the most frequent bigram
        # new_token = len(self.vocab)
        # self.vocab.append(self.vocab[most_frequent_bigram[0]] + self.vocab[most_frequent_bigram[1]])
        # self.merge_rules[most_frequent_bigram] = new_token
 
        # # Step 4: Replace occurrences of the bigram with the new token
        # token_ids = replace_bigram(token_ids, most_frequent_bigram, new_token)

        # return token_ids
        # bigram_counter = Counter()
        # for token_id in token_ids:
        #     bigram_counter = compute_bigram_statistics(token_id, bigram_counter)
        ...
        # return [
        #     replace_bigram(token_id, bigram_to_merge, new_token_id)
        #     for token_id in token_ids
        # ]
        ...
        new_token_id = len(self.vocab)
        merged_token = ''.join([self.vocab[bigram_to_merge[0]], self.vocab[bigram_to_merge[1]]])
    ...
    def encode(self, text: str) -> list[int]:
        ...
        # token_ids = ... 
        # Convert text to list of token ids (character-wise initially)
        # encoded = []
        # for chunk in re.findall(re.compile(r"(\s+\S*)"), text):
        #     token_ids = string_to_ascii(chunk)
        ...
        # # # Keep applying merges until there are no bigrams to merge
        # # previous_token_ids = None
        # # while token_ids != previous_token_ids:
        # #     previous_token_ids = token_ids
        # #     import pdb; pdb.set_trace()
        # #     token_ids = self.merge(token_ids)
