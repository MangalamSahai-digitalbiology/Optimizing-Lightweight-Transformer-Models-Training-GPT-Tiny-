...
class MultiHeadAttention(nn.Module):
    ...
    def q_kT_v(
        ...
        # Divide in multiple heads.
        q = q.view(B, S, self.n_head, D // self.n_head).moveaxis(1,2)
        k = k.view(B, S, self.n_head, D // self.n_head).moveaxis(1,2)
        v = v.view(B, S, self.n_head, D // self.n_head).moveaxis(1,2)
        
        # Transpose the key vector
        kT = k.transpose(-2, -1)
    ...
    def self_attention(
        ...
        """
        causal_mask = torch.tril(torch.ones(q.shape[-2], q.shape[-2], device=q.device)).bool()
        ...
        if attention_mask is None:
            ...
            mask = causal_mask & (expanded_attention_mask.bool())
        ...
        attn = attn_weights @ v  #torch.matmul(attn_weights, v) #(N,H,S,D/H)
        # import pdb; pdb.set_trace()

        attn = attn.transpose(1, 2).contiguous().view(q.shape[0], q.shape[2] , -1)#q.shape[2] * self.n_head)
    ...
    def forward(
        ...
        # Compute Self Attentioon-
        attn = self.self_attention(q, kT, v, attention_mask)

        # Apply projection to attention outputs
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        mha_output = self.mha(x_norm_1, attention_mask)
        x_residual_1 = x_norm_1 + mha_output

        x_norm_2 = self.ln_2(x_residual_1)
        ff_output = self.ff(x_norm_2)

        y = x_residual_1 + ff_output
...
class DecoderLM(nn.Module):
    ...
    def embed(
        ...
        if attention_mask is None:
           position_ids = torch.arange(input_ids.shape[1], dtype=torch.long, device=input_ids.device).unsqueeze(0).expand_as(input_ids)
        else:
